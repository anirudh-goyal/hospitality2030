...
[2026-05-16 23:00] Observation agent expanded to three brief surfaces. Renamed from "Sense Suggested Gesture" to "Sense Observation Agent". Two new custom tools: `add_highlight` (writes to brief.keyFacts) and `add_staff_note` (writes to brief.sensitivities). Per-tool Zod validation in the route; word caps of 6-15 words for body text and 3-10 words for the highlight source string. Agent picks one or more channels per observation, or none if the observation is trivial. New Convex mutations: `appendHighlight`, `appendStaffNote`, each prepending and capping (highlights at 5, staff notes at 5). Verified: agent correctly chose `add_highlight` for "James sold his company" (durable bio, not a gesture trigger), anonymized the source to "Housekeeping, Rosewood Hong Kong - May 16", and skipped the file read + web search since no gesture was being produced (run completed in ~20s instead of 60s). Second run with same observation returned `submitted: false` — agent correctly recognized the highlight was already present and added nothing.
